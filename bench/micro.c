...
static char const * TMPFILE = DEFAULT_TMPFILE;
...
_parse(int argc, char * argv[])
{
  ...
  for (i=1; i<argc; ++i) {
    if (0 == strncmp("--mem=", argv[i], 6)) {
      NUM_MEM = (size_t)atol(argv[i]+6);
    }
    else if (0 == strncmp("--sys=", argv[i], 6)) {
      NUM_SYS = (size_t)atol(argv[i]+6);
    }
  ...
  SIZ_PAG = (unsigned long)sysconf(_SC_PAGESIZE)*NUM_SYS;
...
int main(int argc, char * argv[])
{
  ...
  pgsize = (unsigned long)sysconf(_SC_PAGESIZE);
